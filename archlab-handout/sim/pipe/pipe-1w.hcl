...
word f_predPC =
        [f_icode in{IJXX, ICALL}:f_valC;##1W:Want to refetch popq one time
                1:f_valP;
];

################Decode Stage######################################

        ##W1: Strategy.Decoding of popq rA should be treated the same##as would
                      iaddq $8,
        % rsp##Decoding of pop2 rA treated same as mrmovq - 8(% rsp),
        rA
...
#/* $end pipe-m_stat-hcl */
                                                ...
                                                ##1W
                                                                                        : For this problem,
                                                we introduce a multiplexor that merges##valE and valM into a single value for writing to register port E.##DO NOT CHANGE THIS LOGIC##Merge both write back sources onto register port E##Set E port register ID
                                                        word w_dstE = [##writing from valM
                                                                               W_dstM != RNONE:W_dstM;
                                                                                             1:W_dstE;
];

                                                ##Set E port value word w_valE = [W_dstM != RNONE:W_valM; 1:W_valE;];

                                                ##Disable register port M##Set M port register ID word w_dstM = RNONE;

                                                ##Set M port value word w_valM = 0;
